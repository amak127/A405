...
%Calculating for area of work done under plot.
%Take top plot, find area underneath curve.
%Then find bottom plot and find area underneath this curve.
%Find the difference between the two areas to find the area between
%the two curves. (I'm not too sure if the answer is right or not.)
rhoCB = [rhoCA,flipdim(rhoAB,2)];
pressCB = [press900Vec*0.01,pressLevs*0.01];

%Setting up functions first to use when interpolating the data between
%points on the curves.
function iterpBC = interpBC(x)
    iterpBC = interp1(1./rhoBC,pressLevs*0.01,x);
end

function iterpCB = interpCB(x)
   iterpCB = interp1(1./rhoCB,pressCB,x);
end

%Finding each curve's area with quad function and finally finding
%total work.
areaBC = quad(@interpBC,1/rhoBC(end),1/rhoBC(1));
areaCB = quad(@interpCB,1/rhoCB(1),1/rhoCB(end));
       
totWork = areaCB - areaBC
